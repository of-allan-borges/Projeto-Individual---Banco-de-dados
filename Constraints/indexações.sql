...
CREATE INDEX idx_local_biblioteca ON local(id_local, id_biblioteca); -- Índice em id_local e id_biblioteca na tabela local 
SELECT livro.titulo, local.andar, local.prateleira, 
biblioteca.nome_biblioteca FROM livro JOIN local ON livro.id_local = 
local.id_local JOIN biblioteca ON local.id_biblioteca = 
biblioteca.id_biblioteca WHERE livro.id_livro = 2; -- Exemplo de consulta
